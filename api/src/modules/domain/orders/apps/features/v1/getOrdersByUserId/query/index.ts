...
// #region Query
@sealed
export class GetOrdersByUserIdQuery extends RequestData<
	DataResponse<Array<GetOrdersByUserIdResponseDto>>
> {
...
export class GetOrdersByUserIdQueryHandler
	implements
		RequestHandler<GetOrdersByUserIdQuery, DataResponse<Array<GetOrdersByUserIdResponseDto>>>
{
	...
	public constructor() {
		this._getOrdersByUserIdValidationService = Container.get(
			GetOrdersByUserIdValidationService
		);
		this._getOrdersByUserIdDbService = Container.get(GetOrdersByUserIdDbService);
		this._getOrdersByUserIdMapResponseService = Container.get(
			GetOrdersByUserIdMapResponseService
		);
	}

	public async handle(
		value: GetOrdersByUserIdQuery
	): Promise<DataResponse<GetOrdersByUserIdResponseDto[]>> {
		try {
			// Guard
			if (!value)
			...
			await this.pipeline.step(PipelineSteps.MapResponse, async () => {
				// Get Db Result
				const ordersEntity = this.pipeline.getResult<OrderEntity[]>(
					PipelineSteps.DbService
				);
			...
			// Return
			const response = this.pipeline.getResult<GetOrdersByUserIdResponseDto[]>(
