...
export interface IFetchUserOrderService
	extends IServiceHandlerAsync<IFetchUserOrderServiceParameters, IUserOrdersType> {}

@sealed
@Service()
export class FetchUserOrderService implements IFetchUserOrderService {
	public handleAsync(
		params: IFetchUserOrderServiceParameters
	): Promise<Result<IUserOrdersType, ResultError>> {
		return tryCatchResultAsync(async () => {
			...
			const userPromise: Promise<ReplyMessageBullMq<JsonString>> = producerUser.sendAsync<
				string,
				JsonString
			>(`JOB:${requestUserQueue}`, messageUser);
			...
			const orderPromise: Promise<ReplyMessageBullMq<JsonString>> = producerOrder.sendAsync<
				string,
				JsonString
			>(`JOB:${requestOrderQueue}`, messageOrders);

			// Await All
			const [userMessageResult, orderMessageResult] = await Promise.all([
				userPromise,
				orderPromise,
			]);
